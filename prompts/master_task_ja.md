...
7. 英語版・日本語版ともに、Overleaf でコンパイル可能な `tex/survey_en.tex`, `tex/survey_ja.tex`, `tex/references.bib` を作成してください。
...
12. 原稿完成後、`[$overview-figure](.agents/skills/overview-figure/SKILL.md)` を使って academic style の overview figure を作成してください。図内の言語は英語で構いません。
13. figure auditor の採否を経て、採用された overview figure を本文の適切な位置に挿入し、本文中でも Figure 1 / 図1 として参照・説明してください。
14. 最終的に `python3 scripts/link_markdown_refs.py --run <RUN_DIR>`, `python3 scripts/render_tex_bib.py --run <RUN_DIR>`, `python3 scripts/validate_outputs.py --run <RUN_DIR>` を実行してください。

最終回答では、run directory と主要ファイルを示し、coverage・citation・web integration・figure audit・validation 上の残存リスクを簡潔に報告してください。
